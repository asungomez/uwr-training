...
interface GymMaterialsFieldProps {
  control: Control<ExerciseFormValues>
}

/** Manages an exercise's list of gym materials (equipment). Type a name to get
 *  suggestions from existing materials; pick one to link it, or just add the typed
 *  text to create a new material on save. Each added material is a removable chip. */
function GymMaterialsField({ control }: GymMaterialsFieldProps) {
  const { fields, append, remove } = useFieldArray({ control, name: 'gymMaterials' })
  ...
  const suggestions = (data?.items ?? []).filter(
    (material) => !addedNames.has(material.name.toLowerCase()),
  )
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className={labelClass}>Materiales</span>
      ...
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
              // Don't submit the whole form; add the material instead.
              event.preventDefault()
              add(draft)
            }
          }}
          placeholder="Añadir material (p. ej. Mancuernas)…"
          aria-label="Material"
...
export default GymMaterialsField
